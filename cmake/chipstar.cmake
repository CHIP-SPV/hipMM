...
# Fix build flags by removing conflicting --offload-arch flags from generated build files
function(chipstar_fix_build_flags)
  if(NOT _CHIPSTAR_DETECTED)
    return()
  endif()
  
  # Check if target already exists to avoid duplicate target error
  if(TARGET fix_chipstar_flags)
    return()
  endif()

  # Create Python script to remove conflicting flags (more reliable than CMake regex)
  set(FIX_SCRIPT_CONTENT
    "#!/usr/bin/env python3
import re
import sys
import glob
...
build_files = [\"${CMAKE_BINARY_DIR}/build.ninja\"]
build_files.extend(glob.glob(\"${CMAKE_BINARY_DIR}/CMakeFiles/*/build.make\", recursive=True))
...
for build_file in build_files:
    try:
        ...
        # Remove all --offload-arch flags using regex
        content = re.sub(r'--offload-arch=[^\\s]+', '', content)
        # Remove ROCm include paths
        content = re.sub(r'-I/opt/rocm-[^\\s]+', '', content)
...
print(\"chipStar: Fixed build files: removed --offload-arch flags\")
")
  
  file(WRITE "${CMAKE_BINARY_DIR}/fix_chipstar_flags.py" "${FIX_SCRIPT_CONTENT}")
  file(COPY "${CMAKE_BINARY_DIR}/fix_chipstar_flags.py" DESTINATION "${CMAKE_BINARY_DIR}" FILE_PERMISSIONS OWNER_READ OWNER_WRITE OWNER_EXECUTE GROUP_READ GROUP_EXECUTE WORLD_READ WORLD_EXECUTE)
  
  # Create custom target that runs the fix script automatically
  # This target runs before any compilation to remove --offload-arch flags
  add_custom_target(fix_chipstar_flags ALL
    COMMAND ${CMAKE_COMMAND} -E env PYTHONPATH="" python3 "${CMAKE_BINARY_DIR}/fix_chipstar_flags.py"
    COMMENT "chipStar: Removing conflicting --offload-arch flags"
    VERBATIM)
  
  message(STATUS "chipStar: Created fix_chipstar_flags target to remove conflicting compiler flags")
endfunction()
